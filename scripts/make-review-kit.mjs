...
for (const scene of ep.scenes) {
  ...
  scenes.push({ id: scene.id, start: cursor, end: cursor + duration });
  cursor += duration;
}
...
for (const unit of units) {
  ...
  if (dur(last) < MIN_CHUNK) {
    last.ids.push(...unit.ids);
    last.end = unit.end;
    continue;
  }
  if (dur(unit) < MIN_CHUNK) {
    if (dur(last) + dur(unit) <= MAX_CHUNK) {
      last.ids.push(...unit.ids);
      last.end = unit.end;
    } else {
      packed.push({ ...unit, ids: [...unit.ids] });
...
for (const cut of cuts) {
  ...
  console.log(
    `${cut.name}  ${cut.cutStart.toFixed(1)}-${cut.cutEnd.toFixed(1)}s  ${(statSync(outPath).size / 1024 / 1024).toFixed(2)}MB  still ${still}`,
  );
}
...
writeFileSync(
  ...
  [
    "# Review kit",
    "",
    "Beat-split chunks of `out/ep01.mp4`. 60–150s, under 12MB, 8s overlap.",
    "Last file is `cta`. Seams reel is last 8s plus first 8s of each join.",
    "",
    ...cuts.map(
      (cut) =>
